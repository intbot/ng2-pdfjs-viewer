(function() {
  ...
  console.log('🟢 postmessage-wrapper.js: TEST LOG - BUILD_ID:', '2025-07-10T23-06-31-000Z');
  ...
  function setupBidirectionalEventListeners() {
        ...
        Object.defineProperty(app.pdfViewer, 'currentScale', {
          ...
          set: function(scale) {
            ...
            if (scale !== lastKnownScale) {
              ...
              log(`Zoom changed via currentScale to: ${zoomValue} (scale: ${scale})`);
    ...
    if (app.eventBus) {
      ...
      app.eventBus.on('scalechanging', (event) => {
        log(`Scale changing event received: ${event.scale}`);
        
        // Convert scale to zoom format
        let zoomValue;
        if (typeof event.scale === 'string') {
          zoomValue = event.scale;
        } else if (typeof event.scale === 'number') {
          zoomValue = `${Math.round(event.scale * 100)}%`;
        } else {
          zoomValue = 'auto';
        }
