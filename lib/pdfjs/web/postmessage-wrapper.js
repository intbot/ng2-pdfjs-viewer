(function() {
  ...
  console.log('🟢 postmessage-wrapper.js: TEST LOG - BUILD_ID:', '2025-07-27T24-30-04-000Z');
      ...
      app.eventBus.on('pagerendered', (event) => {
        if (window.ng2PdfJsViewerEnablePageRendered) {
          ...
          const renderData = {
            ...
            source: event.source || 'pagerender',
            timestamp: Date.now()
          };
  ...
  function setupIdleDetection() {
    ...
    // Reset idle timer when PDF document changes
    if (app && app.eventBus) {
      const documentLoadedHandler = () => {
        resetIdleTimer();
      };
      app.eventBus.on('documentloaded', documentLoadedHandler);
      // Note: PDF.js event bus doesn't support removeListener, so we can't track this one for cleanup
    }
  ...
  function interceptOutlineClicks() {
    // Prevent multiple setups
    ...
    // Wait for PDF outline viewer to be available
    const checkOutlineViewer = setInterval(() => {
      if (app && app.pdfOutlineViewer && app.pdfOutlineViewer._bindLink) {
        ...
        clearInterval(checkOutlineViewer);
      }
    }, 100); // Check every 100ms until outline viewer is available

    // Clear interval after 10 seconds if outline viewer is not found
    setTimeout(() => {
      clearInterval(checkOutlineViewer);
    }, 10000);
  }
