(function() {
  ...
  console.log('🟢 postmessage-wrapper.js: TEST LOG - BUILD_ID:', '2025-07-25T21-09-28-000Z');
  ...
  const ControlUtils = {
    ...
    updateModeViaEventBus: function(eventName, modeValue, modeMap, propertyName, notificationProperty) {
      const app = PDFViewerApplication;
      if (!app?.eventBus) {
      ...
      try {
        ...
        if (mode !== undefined) {
          ...
          // Send state change notification for two-way binding
          if (notificationProperty) {
            const originalValue = typeof modeValue === 'string' ? modeValue.toLowerCase() : modeValue;
            sendStateChangeNotification(notificationProperty, originalValue, 'user');
          }
        } else {
