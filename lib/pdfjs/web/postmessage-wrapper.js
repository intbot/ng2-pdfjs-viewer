  ...
  function updateZoom(zoom) {
    ...
    try {
      ...
      if (validStringZooms.includes(zoom)) {
        // These are valid string zoom values
        log(`Using string zoom value: ${zoom}`);
      } else if (typeof zoom === 'number' && zoom >= 0.1 && zoom <= 10) {
        // Valid numeric zoom value
        log(`Using numeric zoom value: ${zoom}`);
      } else if (typeof zoom === 'string' && !isNaN(parseFloat(zoom))) {
        // String that can be converted to number
        const numericZoom = parseFloat(zoom);
        if (numericZoom >= 0.1 && numericZoom <= 10) {
          zoom = numericZoom;
          log(`Converted string zoom to numeric: ${zoom}`);
        } else {
          log(`Zoom scale out of range: ${numericZoom} (valid range: 0.1-10)`, 'warn');
          return;
        }
      } else {
        log(`Invalid zoom value: ${zoom} (type: ${typeof zoom})`, 'warn');
        return;
      }
      
      // For numeric zoom values, ensure they're in the correct format for PDF.js
      if (typeof zoom === 'number') {
        // PDF.js expects zoom values as multipliers (1.0 = 100%, 0.5 = 50%, etc.)
        // The warnings suggest PDF.js might be interpreting our values as percentages
        // Let's ensure we're passing the correct format
        log(`Final numeric zoom value: ${zoom} (should be multiplier, not percentage)`);
      }
      
      if (PDFViewerApplication.eventBus) {
        // Use PDF.js v4.x event bus dispatch for zoom changes
        try {
          // For string zoom values, use the scale selector
          if (typeof zoom === 'string' && validStringZooms.includes(zoom)) {
            // Use the scale selector for predefined zoom values
            const scaleSelect = document.getElementById('scaleSelect');
            if (scaleSelect) {
              scaleSelect.value = zoom;
              scaleSelect.dispatchEvent(new Event('change'));
              log(`Zoom updated via scale selector to: ${zoom}`);
            } else {
            ...
            // For numeric zoom values, use the event bus
            PDFViewerApplication.eventBus.dispatch('scalechanged', {
              value: zoom
            });
            log(`Zoom updated via event bus to: ${zoom}`);
          ...
          log(`Event bus zoom update failed: ${eventError.message}`, 'warn');
        }
      }
      ...
      if (PDFViewerApplication.pdfViewer) {
        try {
          // Try setting the scale value directly
          if (typeof zoom === 'string' && validStringZooms.includes(zoom)) {
            PDFViewerApplication.pdfViewer.currentScaleValue = zoom;
          } else {
            PDFViewerApplication.pdfViewer.currentScale = zoom;
          }
