(function() {
  ...
  function toggleToolbarSectionVisibility(elementId, visible) {
    ...
    log(`Toolbar section ${elementId} visibility set to: ${visible}`);
  }
      ...
      app.initializedPromise.then(() => {
        ...
        if (app.eventBus && typeof app.eventBus.dispatch === 'function') {
          ...
          if (app.pdfViewer && app.pdfCursorTools) {
            ...
            // Apply stored CSS zoom setting if it exists
            if (typeof window.ng2PdfJsViewerUseCssZoom === 'boolean') {
              app.pdfViewer.useOnlyCssZoom = window.ng2PdfJsViewerUseCssZoom;
              log(`Applied stored CSS zoom setting: ${window.ng2PdfJsViewerUseCssZoom}`);
            }
          }
        }
      }).catch(error => {
  ...
  function setDownloadFilename(filename) {
    try {
      if (filename && PDFViewerApplication) {
        ...
        log(`Download filename set to: ${processedFilename}`);
      } else {
  ...
  function setPageBorderColor(color) {
    log(`Page border color set to: ${color}`);
