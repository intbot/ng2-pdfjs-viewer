(function() {
  ...
  console.log('🟢 postmessage-wrapper.js: TEST LOG - BUILD_ID:', '2025-07-26T21-41-16-000Z');
  ...
  function setupBidirectionalEventListeners() {
    ...
    if (app.pdfCursorTools) {
      ...
      if (originalSetActiveTool) {
        ...
        log('Cursor tool setActiveTool method not found - using event-based approach instead', 'info');
      }
    } else {
      log('pdfCursorTools not available - using event-based approach instead', 'info');
    }
    ...
    if (app.pdfViewer) {
      ...
      if (originalSetSpreadMode) {
        ...
        log('pdfViewer.setSpreadMode method not available in PDF.js v5.3.93 - using event-based approach', 'info');
      }
