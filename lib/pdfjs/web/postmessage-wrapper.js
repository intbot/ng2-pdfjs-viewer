(function() {
  ...
  function updateZoom(zoom) {
    ...
    try {
      ...
      // Validate zoom value
      if (zoom === 'auto' || zoom === 'page-actual' || zoom === 'page-fit' || zoom === 'page-width' || zoom === 'page-height') {
  ...
  function updateCursor(cursor) {
    ...
    try {
      ...
      if (PDFViewerApplication.pdfCursorTools) {
        try {
          PDFViewerApplication.pdfCursorTools.switchTool(toolId);
      ...
      // Also try updating the toolbar button state if available
      if (PDFViewerApplication.toolbar) {
        try {
          const toolbarItems = PDFViewerApplication.toolbar.items;
          if (toolbarItems) {
