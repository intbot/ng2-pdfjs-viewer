(function() {
  ...
  console.log('🟢 postmessage-wrapper.js: TEST LOG - BUILD_ID:', '2025-07-10T23-42-10-000Z');
  ...
  function setupBidirectionalEventListeners() {
    ...
    if (app.eventBus) {
      ...
      // Listen for scale changing events (this might be more reliable than method interception)
      app.eventBus.on('scalechanging', (event) => {
        log(`Scale changing event received: ${event.scale} (scaleValue: ${event.scaleValue})`);
        
        // Use enhanced zoom transformation with scale value information
        let zoomValue;
        if (event.scaleValue) {
          // If we have scaleValue, use it directly as it contains the zoom mode
          zoomValue = event.scaleValue;
        } else {
          // Fall back to transformation function
          zoomValue = transformZoomFromViewer(event.scale, app);
        }
        
        log(`Zoom changed via scalechanging event to: ${zoomValue}`);
        sendStateChangeNotification('zoom', zoomValue, 'user');
      });
